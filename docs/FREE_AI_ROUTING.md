...
Only Anthropic, Groq, and Ollama are wired into Albert today. The other rows are researched next-provider
options, not controls that already exist in Systems.
...
| 3 | Gemini `gemini-3.5-flash-lite` | Researched, not implemented | Fast multimodal/vision/file route | Free input and output on the standard free tier, subject to project quotas. Free-tier traffic may be used to improve Google products. |
...
- calls the mixed Ollama/Groq tier **QUICK**, and calls Groq **Groq Cloud**, instead of implying it is
  on-device/private;
...
2. Casual cloud request: Groq `openai/gpt-oss-20b`.
3. Vision, audio input, or very long context: Gemini 3.5 Flash-Lite.
4. Groq quota/outage: Cerebras `gpt-oss-120b`.
